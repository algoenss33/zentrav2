...
const WALLET_CONFIGS: Record<string, { name: string; icon: string }> = {
  metamask: { name: "MetaMask", icon: "/metamask.png" },
  phantom: { name: "Phantom", icon: "/phantom.png" },
  bitget: { name: "Bitget", icon: "/bitget.png" },
  rabby: { name: "Rabby Wallet", icon: "/rabby.png" },
  trustwallet: { name: "Trust Wallet", icon: "/trustwallet.png" },
  okx: { name: "OKX", icon: "/okx.png" },
}

export default function WalletPopup({ open, walletName, walletId, onClose }: WalletPopupProps) {
  ...
  const walletConfig = WALLET_CONFIGS[walletId] || { name: walletName, icon: "/wallet.png" }
  ...
  return (
    <Dialog open={open} onOpenChange={() => {}}>
      ...
      >
        ...
        <DialogTitle className="sr-only">{walletConfig.name} Wallet</DialogTitle>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-yellow-500/20">
          ...
          <h2 className="text-white font-semibold text-base flex-1 text-center">
            {showImport ? walletConfig.name : "Add wallet"}
          </h2>
          <div className="w-9 h-9" />
        </div>
